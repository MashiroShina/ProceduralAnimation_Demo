...
public class CreatureController : MonoBehaviour {
    ...
    void Update () {
        ...
        rSpeed = Mathf.MoveTowards (rSpeed, Input.GetAxis("Vertical") * rotationSpeed, rotateInputFactor * Time.deltaTime);
        transform.Rotate (0f, rSpeed * Time.deltaTime, 0f);
        transform.position += velocity * 1 * Time.deltaTime;
        if (dynamicGait) {
            timeBetweenSteps = maxTargetDistance / Mathf.Max (1 * velocity.magnitude, Mathf.Abs (rSpeed * Mathf.Deg2Rad * averageRotationRadius));
        }
